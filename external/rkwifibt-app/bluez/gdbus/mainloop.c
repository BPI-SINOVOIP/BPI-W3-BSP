						...
						DBusDispatchStatus status)
{
	...
	if (status == DBUS_DISPATCH_DATA_REMAINS) {
		...
		g_source_set_callback (source, message_dispatch, dbus_connection_ref(conn), NULL);
...
static dbus_bool_t add_watch(DBusWatch *watch, void *data)
{
	...
	g_source_set_callback (source, (GSourceFunc)watch_func, info, NULL);
	info->id = g_source_attach (source, Bluez_Context);
	g_source_unref (source);
	g_io_channel_unref(chan);
	G_source = source;
#endif
