...
static XCamReturn AwbDemoPrepare(RkAiqAlgoCom* params)
{
    ...
    if (!algo_ctx->cutomAwbInit) {
        initAwbHwFullConfigWp(&algo_ctx->rkCfg.awbHwConfig);
...
static void WriteMeasureResult(rk_aiq_awb_stat_res_v201_t &awb_measure_result, int log_level ,int camera_id)
{
...
static void WriteDataForThirdParty(const rk_aiq_isp_awb_meas_cfg_v3x_t &wpDetectPara,int log_level ,int camera_id)
{
    if(log_level < XCORE_LOG_LEVEL_LOW1){
        return;
    }
