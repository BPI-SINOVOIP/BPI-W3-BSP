...
)
{
  ...
  for (Index = 0; Index < 0x3000; Index++) {
    DEBUG ((DEBUG_WARN, "{%02x,%02x,%02x,%02x} ", (*(BitMapIn + Index)).Red, (*(BitMapIn + Index)).Green, (*(BitMapIn + Index)).Blue, (*(BitMapIn + Index)).Reserved));
  }
}
  ...
  case LcdBitsPerPixel_24:
    ...
    for (SourceLine = SourceY, DestinationLine = DestinationY;
           ...
           SourcePixelX++, DestinationPixelX++)
      {
        ...
        //DEBUG ((DEBUG_WARN, "(0x%08x) ", DestinationPixel32bit));
        // Copy the pixel into the new target
        ...
        *DestinationPixel32bit = (UINT32) (
              ( (EfiSourcePixel->Red          <<  16))
            | ( (EfiSourcePixel->Green        <<  8))
            | ( (EfiSourcePixel->Blue         <<  0))
            | ( (EfiSourcePixel->Reserved     <<  24))
            );
      }
    }
#endif
